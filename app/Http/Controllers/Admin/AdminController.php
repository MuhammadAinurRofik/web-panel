...
class AdminController extends Controller
{
    ...
    {
        // 1. Ambil data Statistik Proyek dari Database
        $projectStats = [
            'total_users'      => \App\Models\User::where('usertype', 'user')->count(),
        ...
        // 2. Ambil data Resource Sistem (Optimasi Cache 30 detik)
        $systemStats = \Illuminate\Support\Facades\Cache::remember('system_resource_stats', 30, function () {
            // CPU & Core
            ...
            $ramTotal = $mem[1];
            $ramUsed = $mem[2];
            $ramPerc = ($ramUsed / $ramTotal) * 100;

            // Disk (/var/www)
            $diskPath = "/var/www"; 
            $diskTotal = disk_total_space($diskPath);
            $diskUsed = $diskTotal - disk_free_space($diskPath);
            $diskPerc = ($diskUsed / $diskTotal) * 100;

            return [
                'cpu' => [
                    'usage' => round($cpuUsage, 1),
                    'cores' => $cpuCores,
                    'color' => ($cpuUsage > 80) ? 'rose' : (($cpuUsage > 50) ? 'amber' : 'emerald')
                ],
                'ram' => [
                    'used' => round($ramUsed / (1024**3), 2),
                    'total' => round($ramTotal / (1024**3), 2),
                    'percentage' => round($ramPerc, 1),
                    'color' => ($ramPerc > 80) ? 'rose' : 'indigo'
                ],
                'disk' => [
                    'used' => round($diskUsed / (1024**3), 2),
                    'total' => round($diskTotal / (1024**3), 2),
                    'percentage' => round($diskPerc, 1),
                    'color' => ($diskPerc > 85) ? 'rose' : 'emerald'
                ]
            ];
        });

        // 3. Gabungkan semuanya ke variabel $stats
        $stats = array_merge($projectStats, $systemStats);
    ...
    public function terminalExecute(Request $request)
    {
        ...
        $blacklist = ['rm -rf /', 'rm', 'echo', 'touch', 'mkdir', 'shutdown', 'reboot', 'mkfs', ':(){ :|:& };:'];
