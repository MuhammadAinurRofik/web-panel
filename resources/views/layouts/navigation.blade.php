<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ Auth::user()->usertype == 'admin' ? route('admin.dashboard') : route('dashboard') }}" class="flex items-center gap-3 group">
                        <div class="w-10 h-10 bg-orange-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-orange-100 group-hover:rotate-6 transition-transform duration-300">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden shadow-inner bg-white border-t border-gray-100">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="Auth::user()->usertype == 'admin' ? route('admin.dashboard') : route('dashboard')" 
                                    :active="Auth::user()->usertype == 'admin' ? request()->routeIs('admin.dashboard') : request()->routeIs('dashboard')">
                {{ __('Dashboard') }}
            ...
            @if (Auth::user()->usertype == 'admin')
                <x-responsive-nav-link :href="route('admin.projects.index')" :active="request()->routeIs('admin.projects.index')">
                    {{ __('Daftar Antrean') }}
                    @if($pendingCount > 0)
                        <span class="ms-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-600">
                            {{ $pendingCount }}
                        </span>
                    @endif
                </x-responsive-nav-link>

                <x-responsive-nav-link :href="route('admin.projects.active')" :active="request()->routeIs('admin.projects.active')">
                    {{ __('Project Aktif') }}
                </x-responsive-nav-link>
                ...
                <x-responsive-nav-link :href="route('admin.backups.index')" :active="request()->routeIs('admin.backups.*')">
                    {{ __('Backup Manager') }}
                    @if($backupCount > 0)
                        <span class="ms-2 inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-black bg-indigo-100 text-indigo-700 uppercase">
                            {{ $backupCount }}
                        </span>
                    @endif
                </x-responsive-nav-link>

                <x-responsive-nav-link :href="route('admin.terminal')" :active="request()->routeIs('admin.terminal')">
                    <div class="flex items-center">
                        {{ __('Terminal') }}
                        <span class="ms-2 flex h-2 w-2 relative">
        ...
        <div class="pt-4 pb-1 border-t border-gray-100 bg-gray-50/50">
            <div class="px-4 flex items-center justify-between">
                <div>
                    <div class="font-black text-sm text-gray-800 uppercase tracking-tighter">{{ Auth::user()->name }}</div>
                    <div class="font-medium text-xs text-gray-500">{{ Auth::user()->email }}</div>
                </div>
                <span class="text-[9px] font-black px-2 py-1 bg-white border border-gray-200 rounded-lg uppercase tracking-widest text-gray-400">
                    {{ Auth::user()->usertype }}
                </span>
            </div>
